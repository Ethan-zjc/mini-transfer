...
module.exports = function ({ path, context, file }) {
    ...
    if (get(path, 'node.type') === 'ObjectProperty' && get(path, 'node.key.name') === 'relations') {
        ...
        path.traverse({
            ...
            ObjectProperty(path) {
                ...
                if (path.node.key.name === 'type') {
                    ...
                    const action =
                        get(path, 'node.value.value') === 'child'
                            ? 'relationComponentsParent'
                            : 'relationComponentsChild';
        ...
        if (!linkedBody) {
            ...
            path.replaceWith(
                types.objectMethod('method', types.identifier('attached'), [], linkedBody, false)
            );
        }
    }
    let selectMethod = '';
    if (path.node.type === 'ObjectProperty' && path.node.key.name === 'methods') {
        ...
        path.traverse({
            MemberExpression(memberPath) {
                if (
                    get(memberPath, 'node.property.type') === 'NumericLiteral' &&
                    get(memberPath, 'node.property.value') >= 0
                ) {
                    const node = memberPath.node;
                    if (
                        get(node, 'object.type') === 'CallExpression' &&
                        ...
                        memberPath.replaceWith(
                            types.callExpression(
                                get(memberPath, 'node.object.callee'),
        ...
        path.traverse({
            CallExpression(callPath) {
                if (
                    has(callPath, 'node.arguments[0]') &&
                    get(callPath, 'node.arguments[0].type') === 'StringLiteral'
                ) {
                    callPath.traverse({
                        MemberExpression(path) {
                            if (get(path, 'node.property.name') === 'getRelationNodes') {
                                const relationsValue =
                                    get(callPath, 'node.arguments[0].value') || '';
