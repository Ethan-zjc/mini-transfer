...
const { transformCssStaticUrl } = require('./tools');
const SWAN_ID_FOR_SYSTEM = 'swanIdForSystem';
const constant = require('../config/constant');
const transformEnvView = require('./envView');
...
function transformComponent(node, file, context) {
    ...
    Object.keys(relationComponentsParent).forEach((componentKey) => {
        if (componentKey === name || componentKey === (name.slice && name.slice(2))) {
            ...
            //获取依赖数组的第一个元素
            const attribSystem =
                relationComponentsParent[componentKey][0] + '-' + componentUniqueFlag;
    ...
    Object.keys(relationComponentsChild).forEach((componentKey) => {
        if (componentKey === name || componentKey === (name.slice && name.slice(2))) {
            ...
            addNodeAttrib[SWAN_ID_FOR_SYSTEM] =
                componentsSwanIdMap[parentKey] || componentUniqueFlag;
    ...
    if (needSwanIdComponents.includes(dirName) && addNodeAttrib[SWAN_ID_FOR_SYSTEM]) {
        ...
        if (addNodeAttrib.class) {
            addNodeAttrib.class = addNodeAttrib.class.replace(
                /\d{8,}/,
                `{{${SWAN_ID_FOR_SYSTEM}}}`
            );
        }
    ...
    if (
        needSwanIdComponents.includes(name) ||
        needSwanIdComponents.includes(name.slice && name.slice(2))
    ) {
        addNodeAttrib[SWAN_ID_FOR_SYSTEM] = componentUniqueFlag;
        componentUniqueFlag = (Math.random() + '').slice(2);
    }
...
function transformDirective(node, file, context) {
    ...
    //     const newAttribs = Object.keys(attribs).reduce((newAttribs, attrKey) => {
    ...
    //         ...node,
    //         attribs: newAttribs,
    //         singleQuoteAttribs: Object.keys(singleQuoteAttribs).reduce((prev, key) => {
    ...
    const newAttribs = Object.keys(attribs).reduce((newAttribs, key) => {
        ...
        let newKey = key.replace(/^wx:$/, '').replace(/^wx[:\-]/, directives[context.type]);
    ...
    return {
        ...
        attribs: newAttribs,
        singleQuoteAttribs: Object.keys(singleQuoteAttribs).reduce((prev, key) => {
            const newKey = key.replace(/^wx:/, directives[context.type]);
            return {
                ...prev,
                [newKey]: singleQuoteAttribs[key],
            };
