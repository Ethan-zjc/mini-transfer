...
const { getContent, saveFile, getFiles, isJsonStr } = require('../utils/index');
...
exports.transformConfig = async function (context) {
    const files = await getFiles(context.toPath, 'json');
    ...
    const promise = files.map(async (file) => {
        ...
        let result = null;
        result = transformFn(file, content, context);
        !isWx2Wx && (await exports.transformProjectConfig(file, context));
        await saveFile(file, result.contents);

        // 更新进度
        global.emitter.emit('event', { statue: false, type: context.type, fileType: 'JSON' });
...
function isThirdComponents(componentsPath) {
    return /\/?@baidu\//.test(componentsPath);
}
