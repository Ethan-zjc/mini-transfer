...
/**
 * ** getKkbSetMealList 获取kk币充值套餐列表—端外      Get:/v2/kb/recharge_good/list_h5
 * @author:zhilichao
 * yapi: https://yapi.quickcan.com/project/311/interface/api/3071
 * 对接后端: 喻诗祥 / 袁鑫 (付费)/ 春雷
 * @param(参数):             是否必填     备注:
 ...
 * return Promise
 * **/
function getKkbSetMealList({ third_activity = "", code = "", origin_url = "", from = 3 } = {}) {
...
// 多充多送领取奖励接口
const getAwardApi = ({ topic_id = "", accum_level_id = "", activity_id = "" } = {}) => {
    let url = "/v2/kb/accum_activity/receive";
    let host = "pay";
    let data = {
        topic_id,
        activity_id,
...
// 多充多送文案提示
const getAwardInfoApi = () => {
    let url = "/v1/miniactivity/param/config/search";
    let data = {
        id: app.globalData.onRelease ? 45 : 28,
    };
    return util_request({
        url,
        data,
...
// 参加活动
const postParkInActivity = ({ activity_code = "" } = {}) => {
    const url = `/v1/payactivity/charge_qualification/part_in_activity_by_code`;
    const data = {
        activity_code,
    };
    return util_request({
...
module.exports = {
    getKkbSetMealList, // 获取kk币充值套餐列表—端外
    postPayOrder, // 下单_端外
