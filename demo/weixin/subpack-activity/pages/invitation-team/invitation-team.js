...
const page = {
    data: {
        ...
        query: {}, // 地址传参
        path: "/subpack-activity/pages/invitation-team/invitation-team", // 当前路径
        activity_id: "", // 活动id
        ...
        getTeamInfo({
            ...
            .then((res) => {
                ...
                if (code == 200) {
                    ...
                    // 专题区域
                    const { topicsTitle, topicsList, topicsPool, topicsPoolActivity } = this.formatTopics(viva_card);
                    // 规则区域
        ...
        return new Promise((resolve) => {
            postCaptain({
                ...
                .then((res) => {
                    ...
                    this.teamToast(code == 200 ? "助力成功 记得领取奖励哦" : message);
                    resolve();
                })
                .catch((error) => {
    ...
    createAwardTips() {
        ...
        if (status == 2 && !wx.getStorageSync(storageName)) {
    ...
    formatTeam(value) {
        ...
        const teamButtons = buttons.map((item) => {
    ...
    // 格式化专题
    formatTopics(value) {
        const { title = "作品专区", pool_name = "", topics: list = [], topic_pool_activity_name = "" } = value;
        ...
        return {
            ...
            topicsPoolActivity: topic_pool_activity_name,
        };
    },
    // 格式化奖励列表
    ...
    // 专题查看更多
    tapTopicsMore() {
        const { topicsTitle, topicsPool, topicsPoolActivity } = this.data;
        const prod = "https://h5.kuaikanmanhua.com";
        const stag = "https://mini.kkmh.com";
        const host = global.environment == "prod" ? prod : stag;
        const url = `${host}/act_202101_mini_topic_list.html?title=${topicsTitle}&pool_name=${topicsPool}&activity_name=${topicsPoolActivity}`;
        util_action({
            type: 19,
            url,
        });
    ...
    tapAwardBtn(event) {
        ...
        postAssign({
            ...
            .then((res) => {
                let toast = res.code == 200 ? "奖励已生效 去阅读漫画吧" : res.message;
